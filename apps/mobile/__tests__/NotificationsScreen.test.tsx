...
describe('NotificationsScreen', () => {
  ...
  it('renders notifications and opens the linked screen when tapped', async () => {
    mockFetchNotifications.mockResolvedValueOnce([
      {
        id: 'notif-1',
        type: 'review_reply',
        title: 'Owner da phan hoi review cua ban',
        message: 'Owner Demo vua gui phan hoi cho review tai Lantern Cafe.',
        payload: {
          screen: 'All Reviews',
          params: {
            placeId: 'place-1',
    ...
    await waitFor(() => {
      expect(screen.getByText('Owner da phan hoi review cua ban')).toBeTruthy();
      expect(screen.getByText('Mới')).toBeTruthy();
    });

    fireEvent.press(screen.getByText('Owner da phan hoi review cua ban'));
